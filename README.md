...
**OG 이미지**는 블로그 글 링크를 카카오톡·X·슬랙·페이스북 등에 붙여넣을 때 뜨는
**미리보기 카드 이미지**입니다. (권장 크기 1200×630)

### 지금은 자동으로 만들어집니다 — 따로 안 해도 됩니다

- 글마다 **"제목 + by 작성자"** 카드가 자동 생성됩니다. (`/posts/<slug>/index.png`)
- 홈/사이트는 **"제목 + 설명 + 도메인"** 카드가 자동 생성됩니다. (`/og.png`)
- 색은 허니 테마를 따릅니다.

### 바꾸고 싶다면

**1) 특정 글만 다른 이미지로** — 그 글 frontmatter에 이미지를 지정하면 자동 카드 대신 이 이미지를 씁니다.

```md
---
title: 글 제목
ogImage: ./cover.png # 글과 같은 폴더에 둔 이미지 파일
# 또는: ogImage: https://example.com/cover.png
---
```

**2) 사이트 기본 이미지를 고정 이미지로** — `public/` 에 1200×630 이미지(예: `default-og.jpg`)를 넣고, `astro-paper.config.ts` 의 `site.ogImage` 를 그 파일명으로 지정합니다. (지금은 `default-og.jpg`로 적혀 있지만 파일이 없어 자동 생성 카드를 사용 중입니다. 파일을 넣으면 그게 기본 이미지가 됩니다.)

**3) 자동 카드 디자인 바꾸기 (고급)** — 홈/사이트 카드는 `src/pages/og.png.ts`, 글 카드는 `src/pages/posts/[...slug]/index.png.ts` 에 색·레이아웃·글자 크기가 들어 있습니다.

> 자동 생성을 아예 끄려면 `astro-paper.config.ts` 의 `features.dynamicOgImage` 를 `false`로 둡니다. 그 경우 항상 `public/` 의 고정 이미지를 쓰며, 그 파일이 반드시 있어야 합니다.
